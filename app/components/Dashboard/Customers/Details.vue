<!-- store-app/components/Dashboard/Customers/Details.vue -->
<template>
  <div>
    <SharedUiHeaderPage
    ...
    <div v-else-if="customer" class="profile-grid">
      ...
      <div class="profile-sidebar">
        ...
        <!-- Delete customer — only shown with canDelete -->
        <SharedUiButtonBase
          v-if="canDelete"
          class="w-100"
...
const { getCustomerById, saveCustomer, markDuePaid, deleteCustomer } =
  useStore();
...
const doDelete = async () => {
  deleting.value = true;
  const r = await deleteCustomer(route.params.id);
  if (r.ok) {
